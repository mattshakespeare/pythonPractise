...
def viewart():
    ...
    for x in cursor.fetchall():
        newrecord = newrecord = str(x[0]) + ", " + str(x[1]) + ", " + str(x[2]) + ", " + str(x[3]) + ", £" + str(x[4]) + "\n"
        outputwindow.insert(END,newrecord)

def searchartistoutput():
    selectedartists = selectedartists.get()
    cursor.execute("SELECT name FROM Artists WHERE artististid=?", [selectedartist])
    for x in cursor.fetchall():
...
def searchmediumoutput():
    ...
    cursor.execute("""SELECT Art.pieceid, Artist.name, Art.title, Art.medium, Art.price
    FROM Artist,Art WHERE Artist.artistid AND art.medium=?""", [selectedmedium])
    for x in cursor.fetchall():
        newrecord = newrecord = str(x[0]) + ", " + str(x[1]) + ", " + str(x[2]) + ", " + str(x[3]) + ", £" + str(x[4]) + "\n"
        outputwindow.insert(END,newrecord)
    medium2.set("")
...
FROM Artists, Art WHERE Artists.artistsid=Art.artististid AND Art.price>=? Art.price <= ?""", [minprice,maxprice])        
    for x in cursor.fetchall():
        newrecord = newrecord = str(x[0]) + ", " + str(x[1]) + ", " + str(x[2]) + ", " + str(x[3]) + ", £" + str(x[4]) + "\n"
        outputwindow.insert(END,newrecord)
...
artistid interger PRIMARY KEY, name text, adress text, town text, county text, postcode text);""")

cursor.execute("""CREATE TABLE IF NOT EXISTS Art(
pieceid interger PRIMARY KEY, artistid interger, title text, medium text, price interger);""")
...
addbtn.place(x = 110, y = 80, width = 100, height = 25)
...
arttitlelbl.place(x = 280, y = 120, width = 80, height = 25)
arttitle = Entry(text = "")
...
artmedium = OptionMenu(window, medium, "Oil", "Watercolour", "Acrylic", "Ink")
...
searchmedium = artmedium = OptionMenu(window, medium, "Oil", "Watercolour", "Acrylic", "Ink")
searchmedium.place(x = 1125, y = 330, width = 100, height = 25)
